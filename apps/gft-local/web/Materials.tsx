...
export interface Material {id:string;topicId:string;name:string;size:number;received:number;processed:number;batches:number;state:string;error?:string;kind:string;pendingEdits?:number[]}
...
export function Materials({topicId}:{topicId:string|null}) {
  ...
  const control=async(job:Material,action:'pause'|'resume')=>{
    if(job.state==='uploading') {if(uploads.has(job.id)){uploads.get(job.id)?.abort();void refresh();}else{resumeId.current=job.id;files.current?.click();}return;}
    mutations.current.add(job.id);setJobs(current=>current.map(j=>j.id===job.id?{...j,state:action==='pause'?'paused':'queued',error:undefined}:j));setError('');
  ...
  const processing=jobs.some(j=>['running','queued'].includes(j.state)||(j.state==='uploading'&&uploads.has(j.id)));
  const controlAll=async()=>{
    const affected=jobs.filter(j=>processing?['running','queued'].includes(j.state)||(j.state==='uploading'&&uploads.has(j.id)):['paused','failed'].includes(j.state));
    ...
    {opened&&<LocalDialog title={tr('主题材料')} wide onClose={()=>{void save().then(ok=>{if(ok){setOpened(false);setPage(null);}});}}>
      ...
      {!page?<>
        ...
        <ul className="gft-material-list">{jobs.map(job=><li key={job.id}>
          <div><strong>{job.name}</strong><span>{tr(job.state==='uploading'&&!uploads.has(job.id)?'等待续传':labels[job.state]||job.state)} · {percentage(job)}%</span></div>
          ...
          <div className="gft-material-buttons">
            {(job.state!=='completed'||!!job.pendingEdits?.length)&&<button onClick={()=>void control(job,['running','queued'].includes(job.state)?'pause':'resume')}>{tr(['running','queued'].includes(job.state)?'暂停':job.state==='uploading'?(uploads.has(job.id)?'暂停':'选择原文件续传'):job.pendingEdits?.length?'应用修正并继续':'继续')}</button>}
