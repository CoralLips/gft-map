...
import { getExamples, exampleBundle } from './examples';
import { translator, type ShowcaseLanguage } from './locale';

/** Browser-only host for the real shared workspace. No local service or model requests. */
export function createShowcaseRuntime(initial: string, notify: (message: string) => void, lang: ShowcaseLanguage = 'zh') {
  const examples = getExamples(lang);
  const tx = translator(lang);
  const maps = new Map<string, PersistedThinkingMap>();
  const listeners = new Set<() => void>();
  const key = lang === 'zh' ? 'gft-map:showcase:v1' : 'gft-map:showcase:v1:en';
  let projects = examples.map(example => ({ id: example.id as string, name: example.name as string, status: 'active' as const }));
  for (const example of examples) maps.set(example.id, bundleToMap(exampleBundle(example, lang), example.id));
  try {
  ...
  let snapshot: MapHostSnapshot = {
    ...
    currentSession: null, chatHistory: [], sourceLabel: tx("示例材料"), showMemoryToggle: false,
    hasSourceMaterial: true,
    redrawDescription: tx("在线示例不连接模型。安装本地版后，重画会按当前主题重新筛选 Log，生成新的 Doc 和 Map。"),
  };
  ...
  const persist = () => {
    ...
    catch { if (!storageWarning) { storageWarning = true; notify(tx("浏览器暂时无法保存这份试改。当前页面仍可使用，请导出脉络包保留修改。")); } }
  };
  const flush = () => { store.getState().flushDocEdits(); store.getState().flushDoc(); };
  const modelNotice = () => notify(tx("这是可编辑的浏览器示例，不连接模型或本机聊天。安装本地版后，可执行更新、整理和重画。"));
  const host: ThinkingMapHost = {
    ...
    async deleteProject(id) {
      flush();
      if (snapshot.projects.length === 1) { notify(tx("至少保留一份示例；可以用“恢复本例”重新开始。")); return; }
    ...
    async importBundle(input) {
      ...
      notify(tx("已导入为一份新脉络，原来的示例仍保留。文件只在此浏览器中读取。"));
    },
  };
  const noModel = async (): Promise<never> => { throw new Error(tx("在线示例不运行模型。请安装本地版后使用这项操作；当前修改已保留。")); };
  ...
  return {
    ...
    async reset(id: string) {
      ...
      flush(); maps.set(id, bundleToMap(exampleBundle(example, lang), id));
