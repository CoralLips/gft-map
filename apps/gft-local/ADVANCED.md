...
数据文件和服务留在本机。选择联网 Agent 时，任务所需内容会交给该 Agent 的模型服务；图文处理不调用 GFT 云端生成 API。可选的 GFT 账号登录只访问授权服务。
...
- 本地与 GFT 平台共用 `gft-theme` v2 格式，仍支持旧 v1 包。单个包上限 4 MB，超过时明确报错，不截断来源。导入导出属于用户主动搬运文件，没有自动云端同步。

## GFT 账号（可选）

在设置中选择“GFT 账号”，打开 GFT 网页完成授权，回到本地即可看到登录账号。登录不上传主题、聊天或 Log；未登录也能使用全部本地功能。授权回调使用短时、一次性随机校验，本地保存独立登录凭证，退出仅清除本应用的登录。
